...
function Profile(props) {
  ...
  function validateButton(email, name) {
    if (email.length >= 2 && EMAIL_REGEXP.test(email) && name.length >= 2 && name.length <= 30 && (currentUser.name !== name || currentUser.email !== email)) {
  ...
  return (
    <section className="profile">
      ...
      <main>
        ...
        {props.isRed ? (
          <button disabled={props.isDisabled} onClick={handleSubmit} type='submit' className={props.isDisabled ? 'profile__save login__save_disabled' : "profile__save"}>Сохранить</button>
        ) : (
          <>
          <button className='profile__edit' type='button' onClick={handleChange}>Редактировать</button>
